...
$$ E_{\rm total} = \sum_{\rm bonds} K_r (r - r_{\rm eq})^2 + \sum_{\rm angles} K_\theta (\theta - \theta_{\rm eq})^2 + \sum_{\rm dihedrals} \frac{V_n}{2} \left[1 + \cos(n\phi - \gamma)\right] + \sum_{i<j} \epsilon_{ij} \left[ \left( \frac{\sigma_{ij}}{R_{ij}} \right)^{12} - \left( \frac{\sigma_{ij}}{R_{ij}} \right)^6 \right] + \sum_{i<j} \frac{q_i q_j}{4 \pi \epsilon_0 R_{ij}} $$
### TODO
